...
var TopHatProblemReporter = GObject.registerClass(
    class TopHatProblemReporter extends PanelMenu.Button {
        _init() {
            ...
            let label = new St.Label({ text: 'Something went wrong while starting TopHat', style_class: 'problem-header' });
            label.clutter_text.line_wrap = true;
            statusMenu.box.add_child(label);
            
            this.msg = new St.Label({ text: '', style_class: 'problem-msg' });
            this.msg.clutter_text.line_wrap = true;
            statusMenu.box.add_child(this.msg);

            label = new St.Label({ text: 'Error details', style_class: 'problem-details-header' });
            statusMenu.box.add_child(label);

            this.details = new St.Label({ text: '', style_class: 'problem-details' });

...
var NetMonitor = GObject.registerClass({
    Properties: {
        'network-unit': GObject.ParamSpec.string(
            'network-unit',
            'Network unit',
            'Display network usage in bits or bytes',
        ...
        _init(settings) {
            ...
            let icon = new St.Icon({gicon, icon_size: 16, style_class: 'system-status-icon tophat-panel-icon tophat-panel-icon-net'});
            this.add_child(icon);
            ...
            settings.bind('meter-fg-color', this, 'meter-fg-color', Gio.SettingsBindFlags.DEFAULT);
            settings.bind('network-usage-unit', this, 'network-unit', Gio.SettingsBindFlags.DEFAULT);
        }
        ...
        refreshCharts() {
            ...
            let netIn = bytesToHumanString(Math.round(bytesInDelta / timeDelta), this.network_unit);
            let netOut = bytesToHumanString(Math.round(bytesOutDelta / timeDelta), this.network_unit);
...
// (e.g., 1024 -> 1 K)
function bytesToHumanString(bytes, netUnit) {
    let bw = bytes;
    let unit = 'B';
    if (netUnit === 'bits') {
        bw *= 8;

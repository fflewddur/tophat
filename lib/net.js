    ...
    class TopHatNetIndicator extends PanelMenu.Button {
        _init() {
            ...
            vbox.connect('notify::vertical', obj => {
                log('[TopHat] Setting network box layout back to vertical orientation');
                obj.vertical = true;
            });
            hbox.add_child(vbox);
        ...
        _buildMenu() {
            ...
            let row = 0;
            let label = new St.Label({ text: 'Network usage', style_class: 'menu-header' });
            lm.attach(label, 0, row, 2, 1);
            row++;

            label = new St.Label({ text: 'Sending:', style_class: 'menu-label' });

...
var Meter = GObject.registerClass({
    ...
    _init(numBars = 1) {
        ...
        for (let i = 0; i < numBars; i++) {
            bars[i] = new St.Widget({style_class: 'tophat-meter-bar', y_align: Clutter.ActorAlign.END, y_expand: false});
            bars[i].save_easing_state();
            bars[i].set_easing_mode(Clutter.AnimationMode.EASE_IN_OUT_CUBIC);
            bars[i].set_easing_duration(500);
    ...
    updateBars(newHeights) {
        for (let i = 0; i < newHeights.length; i++) {
            // log(`TopHat set_height for bar ${i}`);
            this.bars[i].set_height(newHeights[i]);
        }
    }

    setBarWidth(width) {
        this.bars.forEach(b => {
            b.set_style(`width:${width}px`);
        });
    }
});

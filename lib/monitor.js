...
var TopHatMonitor = GObject.registerClass({
    Properties: {
        ...
        'meter-bar-width': GObject.ParamSpec.int(
            'meter-bar-width',
            'Meter bar width',
            'The width for each meter bar in pixels',
            GObject.ParamFlags.READWRITE,
            1, 100,
            1
        ),
...
var Meter = GObject.registerClass({
            ...
            let bar = new St.Widget(
                {
                    ...
                    y_expand: false,
                    style: `height:1px;width:${this.barWidth}px;background-color:${this.meterFGColor};`,
                });
            bars[i] = bar;
    ...
    setUsage(usage) {
        ...
        for (let i = 0; i < usage.length; i++) {
            ...
            let style = `height:${fillHeight}px;width:${this.barWidth}px;background-color:${this.meterFGColor};`;

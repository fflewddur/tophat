...
var Meter = GObject.registerClass({
            ...
            let bar = new St.Widget(
                {
                    ...
                    y_expand: false,
                    style: `background-color:${Config.METER_FG_COLOR};`,
                });

            bar.save_easing_state();
            bar.set_easing_mode(Clutter.AnimationMode.EASE_IN_OUT_CUBIC);
            bar.set_easing_duration(500);
            bars[i] = bar;
    ...
    setBarWidth(width) {
        this.bars.forEach(b => {
            b.set_style(`width:${width}px;background-color:${Config.METER_FG_COLOR};`);
        });
    }
});

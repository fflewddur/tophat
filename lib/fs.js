    ...
    refreshIO() {
        ...
        processes.forEach(pid => {
            ...
            if (procInfo.cmd) {
                ...
                // if (procInfo.hasRecentActivity()) {
                //     log(`cmd=${procInfo.cmd} read=${procInfo.readRate()} read=${Shared.bytesToHumanString(procInfo.readRate())}/s write=${procInfo.writeRate()} write=${Shared.bytesToHumanString(procInfo.writeRate())}/s`);
                // }
            }
        });
        ...
        // procList.forEach(p => log(`cmd=${p.cmd} read=${Shared.bytesToHumanString(p.readRate())}/s write=${Shared.bytesToHumanString(p.writeRate())}/s (read=${p.readRate()} and write=${p.writeRate()})`));
        for (let i = 0; i < Config.N_TOP_PROCESSES; i++) {
            this.topProcesses[i].cmd.text = procList[i].cmd;
            let activity = '';
            if (procList[i].cmd) {
                activity = `Read: ${Shared.bytesToHumanString(procList[i].readRate())}/s Written: ${Shared.bytesToHumanString(procList[i].writeRate())}/s`;
            }
            this.topProcesses[i].usage.text = activity;
        }

...
class ProcDiskActivity {
    ...
    readRate() {
        if (this.readPrev > 0) {
            return (this.read - this.readPrev);
        }
        return 0;
    }
    ...
    writeRate() {
        if (this.writePrev > 0) {
            return (this.write - this.writePrev);
        }
        return 0;
    }
}
    ...
    refreshIO() {
        ...
        processes.forEach(pid => {
            ...
            if (procInfo.cmd) {
                ...
                procInfo.updateActivity(io.disk_rbytes, io.disk_wbytes);
                updatedProcesses.set(pid, procInfo);
                // log(`cmd=${procInfo.cmd} read=${procInfo.readRate()} write=${procInfo.writeRate()}`);
            }
        });

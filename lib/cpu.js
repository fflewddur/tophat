...
class ProcessCPUUse {
    ...
    cpuUsage() {
        ...
        return this.cpuTime() / this.freq * Config.SECOND_AS_MILLISECONDS / Config.UPDATE_INTERVAL_PROCLIST;
    }
...
var TopHatCpuIndicator = GObject.registerClass(
    class TopHatCpuIndicator extends PanelMenu.Button {
        ...
        _buildMenu() {
            ...
            let row = 0;
            let label = new St.Label({ text: 'Processor usage', style_class: 'menu-header' });
            lm.attach(label, 0, row, 2, 1);
            row++;

            label = new St.Label({ text: 'Total CPU:', style_class: 'menu-label' });

...
var CpuMonitor = GObject.registerClass({
    ...
    _buildMenu() {
        ...
        this.menuCpuModel = new St.Label({text: 'model n/a', style_class: 'menu-label-details'});
        this.addMenuRow(this.menuCpuModel, 0, 2, 1);
        label = new St.Label({text: _('Current frequency:'), style_class: 'menu-label-details'});
        this.addMenuRow(label, 0, 1, 1);
        this.menuCpuFreq = new St.Label({text: 'freq n/a', style_class: 'menu-value-details'});
    ...
    _readCPUInfo() {
        new FileModule.File('/proc/cpuinfo').read().then(lines => {
            ...
            // FIXME: Multiply CPU model by number of processors detected
            // FIXME: Average frequency across all processors
            cpuInfo.forEach(info => {
                // log(`processor id: ${info.id} freq: ${(info.freq / info.cores / 1000).toFixed(1)} GHz model: ${info.model}`);
                this.menuCpuModel.text = info.model;
                this.menuCpuFreq.text = `${(info.freq / info.cores / 1000).toFixed(1)} GHz`;
            });
        }).catch(err => {

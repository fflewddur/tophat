...
class ProcessCPUUse {
    ...
    cpuUsage() {
        ...
        return this.cpuTime() / this.freq * Config.SECOND_AS_MILLISECONDS / Config.UPDATE_INTERVAL_CPU;
    }
    ...
    class TopHatCpuIndicator extends PanelMenu.Button {
        ...
        _buildMenu() {
            ...
            for (let i = 0; i < Config.N_TOP_PROCESSES; i++) {
                ...
                lm.attach(cmd, 0, 2 + i, 1, 1);
                let usage = new St.Label({ text: '', style_class: 'menu-cmd-usage' });
                lm.attach(usage, 1, 2 + i, 1, 1);
                let p = new TopProcess(cmd, usage);
        ...
        refresh() {
            ...
            for (let i = 0; i < Config.N_TOP_PROCESSES; i++) {
                ...
                let cpuUse = '';
                if (procList[i].cmd) {
                    cpuUse = Math.round(procList[i].cpuUsage() * 100 / this.cpuCores) + '%';
                }

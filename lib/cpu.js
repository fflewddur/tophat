...
var CpuMonitor = GObject.registerClass({
    ...
    _findTempMonitors() {
        ...
        new FileModule.File(basePath).list().then(files => {
            for (let file of files) {
                ...
                new FileModule.File(path).read().then(name => {
                    // CPU should be named 'coretemp'
                    if (name === 'coretemp') {
                        // determine which processor (socket) we are dealing with
                        new FileModule.File(`${basePath}${file}/temp1_label`).read().then(prefix => {
                            let values = '', id = 0;
                            if ((values = prefix.match(/Package id\s*(\d+)/))) {
                                id = parseInt(values[1]);
                            }
                            let inputPath = `${basePath}${file}/temp1_input`;
                            this.cpuTempMonitors.set(id, inputPath);
                        }).catch(err => {
                            log(`[${Me.metadata.name}] Error reading ${basePath}${file}/temp1_label: ${err}`);
                        });
                    }
                }).catch(err => {
    ...
    _buildCPUDetailRows() {
        ...
        new FileModule.File('/proc/cpuinfo').read().then(lines => {
            ...
            for (let i = 0; i < cpus.size; i++) {
                ...
                this.menuCpuDetails.attach(label, 0, 2, 1, 1);

                label = new St.Label({
                    text: 'n/a',
                    style_class: 'menu-value menu-details menu-section-end',
                });
                this.menuCpuDetails.attach(label, 1, 2, 1, 1);
    ...
    refreshProcesses() {
        ...
        if (this.cpuTempMonitors.size > 0) {
            this._readCPUTemps();
        }

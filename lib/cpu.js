...
var CpuMonitor = GObject.registerClass({
    ...
    _init(configHandler) {
        ...
        id = this.connect('notify::refresh-rate', () => {
            log(`notify::refresh-rate value=${this.refresh_rate}`);
            this._stopTimers();
            this._startTimers();
        });
        this._signals.push(id);
    ...
    _startTimers() {
        ...
        if (this.refreshChartsTimer === 0) {
            this.refreshChartsTimer = GLib.timeout_add(GLib.PRIORITY_DEFAULT, this._computeUpdateInterval(), () => this._refreshCharts());
        }

...
var CpuMonitor = GObject.registerClass(
    class TopHatCpuMonitor extends Monitor.TopHatMonitor {
        _init(settings) {
            ...
            settings.bind('show-animations', this, 'show-animation', Gio.SettingsBindFlags.DEFAULT);
        }
        ...
        _buildMeter() {
            ...
            this.meter = new Monitor.Meter(numBars);
        ...
        refreshCharts() {
            ...
            // Update panel meter
            let height = this.meter.get_height();
            let fillHeight = [];
            if (Config.CPU_SHOW_CORES) {
                fillHeight = new Array(this.cpuCores);
                for (let i = 0; i < this.cpuCores; i++) {
                    fillHeight[i] = Math.ceil(this.cpuCoreUsage[i].total() / 100.0 * height);
                }
            } else {
                fillHeight = [Math.ceil(this.cpuUsage.total() / 100.0 * height)];
            }
            this.meter.updateBars(fillHeight);

...
class ProcessCPUUse {
    ...
    toString() {
        return `{${this.pid} ${this.cmd} ${this.cpuTime()} ${this.freq}}`;
    }
}
    ...
    class TopHatCpuIndicator extends PanelMenu.Button {
        ...
        refresh() {
            ...
            let processes = GTop.glibtop_get_proclist(
                extra_info, GTop.GLIBTOP_KERN_PROC_ALL, GTop.GLIBTOP_EXCLUDE_IDLE
            );
            // log(`[TopHat] ${extra_info}`);
            ...
            processes.forEach(pid => {
                ...
                // log(`[TopHat] procInfo: ${procInfo}`);

                // TODO: refactor into separate function
                let state = new GTop.glibtop_proc_state;
                GTop.glibtop_get_proc_state(state, pid);
                if (state && state.cmd) {
                    let endOfString = state.cmd.indexOf(0);
                    if (endOfString >= 0) {
                        let numCmd = state.cmd.slice(0, endOfString);
                        let cmd = String.fromCharCode.apply(null, numCmd);
                        if (cmd) {
                            // log(`[TopHat] cmd: '${cmd}'`);
                            if (procInfo.cmd && procInfo.cmd != cmd) {
                                // FIXME: Looks like the PID was reused, consider this a new process
                            } else if (!procInfo.cmd) {
                                procInfo.cmd = cmd;
                            }
                        }
                    }
                ...
                procInfo.cpuTimePrev = procInfo.cpuTimeNow;
                procInfo.cpuTimeNow = time.rtime;
                procInfo.freq = time.frequency;
                // procInfo.cpuTime = procInfo.cpuTimeNow - procInfo.cpuTimePrev;
            ...
            procList.sort((a, b) => { return b.cpuTime() - a.cpuTime(); });
            procList = procList.slice(0, 5);
            log(`[TopHat] ${procList}`);

            // Update UI
            // log(`[TopHat] CPU: ${this.cpuUsage}% on ${this.cpuCores} cores (${this.cpuCoreUsage.join()})`);
            // this.valueCPU.text = `${this.cpuUsage}%`;
            this.menuCpuUsage.text = `${this.cpuUsage.total()}%`;

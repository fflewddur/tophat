...
class ProcessMemUse {
    ...
    memUsage() {
        return ((this.resident - this.share) / KB_PER_GB).toFixed(1);
    }
    ...
    class TopHatMemMonitor extends Monitor.TopHatMonitor {
        ...
        _readMemInfo() {
            new FileModule.File('/proc/meminfo').read().then(lines => {
                ...
                this.menuMemUsage.text = `${this.memUsage.mem * 100}%`;
                ...
                this.menuSwapUsage.text = `${this.memUsage.swap * 100}%`;
        ...
        _buildMenu() {
            ...
            this.menuMemSize = new St.Label({text: 'size n/a', style_class: 'menu-value menu-value-details'});
            ...
            this.menuSwapSize = new St.Label({text: 'size n/a', style_class: 'menu-value menu-value-details'});
        ...
        refreshCharts() {
            if (this.hasProc) {
                ...
                // log(`[TopHat] total=${this.mem.total / KB_PER_GB} used=${this.mem.used / KB_PER_GB} free=${this.mem.free / KB_PER_GB} cached=${this.mem.cached / KB_PER_GB} buffer=${this.mem.buffer / KB_PER_GB} shared=${this.mem.shared / KB_PER_GB} user=${this.mem.user / KB_PER_GB}`);
        ...
        repaintHistory() {
            ...
            log(`repaintHistory() xStart=${xStart} height=${height}`);
            for (let i = 0; i < this.history.length; i++) {
                log(`i=${i}, mem = ${this.history[i].mem}`);
            ...
            log(`lineTo ${xStart + (this.history.length - 1) * pointSpacing} height=${height}`);
            ctx.closePath();
            ctx.fill();

...
var PowerMonitor = GObject.registerClass({
    ...
    _init(configHandler) {
        ...
        // Initialize upower
        const upower_client = UPowerGlib.Client.new();
        const upower_devices = upower_client.get_devices();

        let battery_device = null;
        for (const dev of upower_devices) {
        ...
        this.netPrev = {
            power: battery_device.energy_rate,
            battery: battery_device.percentage,
        };
    ...
    refreshCharts() {
        ...
        let power = Math.round(battery_device.energy_rate);
        let battery = Math.round(battery_device.percentage);
        let state = battery_device.state;
    ...
    repaintHistory() {
        ...
        for (const powerUse of this.history) {
            if (powerUse.power > max_power) {
                max_power = powerUse.power;
            }
        }
        ...
        for (let i = 0; i < this.history.length; i++) {
            let pointHeight = Math.ceil(this.history[i].power / max_power * height);

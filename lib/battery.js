...
var PowerMonitor = GObject.registerClass({
    ...
    getPowerValues() {
        const upowerDevices = this.upowerClient.get_devices();
        ...
        if (batteryDevice === null) {
            ...
            //TODO handle this case otherwise crash
        }
        const path = batteryDevice.native_path;
        const basePath = `/sys/class/power_supply/${path}`;
        const currentPath = `${basePath}/current_now`;
        const voltagePath = `${basePath}/voltage_now`;
        const chargePath = `${basePath}/charge_now`;
        const chargeFullPath = `${basePath}/charge_full`;
        const statusPath = `${basePath}/status`;

        const current = this.parseValue(currentPath)*1e-6;
        const voltage = this.parseValue(voltagePath)*1e-6;
        const charge = this.parseValue(chargePath);
        const chargeFull = Math.max(this.parseValue(chargeFullPath), 1); //avoid division by zero

        const status = this.parseStatus(statusPath).replace(/[\n\r]/g, '').toUpperCase();

...
export class CpuMonitor {
    ...
    public start() {
        console.log(`[TopHat] Starting CPU monitor for ${this.model.name} w/ ${this.model.cores} logical cores`);
    ...
    private parseStat(stat: string) {
        ...
        lines.forEach((line: string) => {
            if (line.startsWith('cpu')) {
                const re = /^cpu(\d*)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/;
                const m = line.match(re);
                if (m && !m[1]) {
                    // These are aggregate CPU statistics
                    const usedTime = parseInt(m[2]) + parseInt(m[3]) + parseInt(m[4]) + parseInt(m[6]) + parseInt(m[7]) + parseInt(m[8]) + parseInt(m[9]) + parseInt(m[10]);
                    ...
                    // console.log(`core ${core} usage: ${usage}`);
                }
            }
        });
...
class CpuState {
    ...
    public coreUsageSince(prevState: CpuState, core: number) {
        const usedTimeDelta = this.coreUsedTime[core] - prevState.coreUsedTime[core];
        const idleTimeDelta = this.coreIdleTime[core] - prevState.coreIdleTime[core];
        const usage = usedTimeDelta / (usedTimeDelta + idleTimeDelta);
        return usage;
    }
}
